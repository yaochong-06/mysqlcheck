...
def get_mysql_doc_remote_linux(company_name, engineer_name, customer_name, customer_name2, server_id, server_user,
    ...
    # 1.2 系统磁盘空间使用
    fs_set = command(server_id, server_user, server_password, server_port, "df -hP").split('\n')
    space_param = []
    ...
    context = {'company_name': company_name,
               ...
               # 1.3 系统CPU参数
               # 物理CPU个数
               'p_cpu_num': remove_last_line(command(server_id, server_user, server_password, server_port,
                                                     "cat /proc/cpuinfo |grep 'physical id'|sort |uniq|wc -l")),
               # 逻辑CPU个数
               'l_cpu_num': remove_last_line(command(server_id, server_user, server_password, server_port,
                                                     "cat /proc/cpuinfo |grep 'processor'|wc -l")),
               # CPU核心数
               'cpu_cores': remove_last_line(command(server_id, server_user, server_password, server_port,
                                                     "cat /proc/cpuinfo |grep 'cores'|uniq|awk '{print $4}'")),
               # 每个物理CPU的核数
               'core_per_p': remove_last_line(command(server_id, server_user, server_password, server_port,
    ...
    tpl.render(context)
    tpl.save(f'./{server_id}:{mysql_port}:{current_time}-{business_name}.docx')
...
# get_mysql_doc_remote_win('company_name', 'engineer_name', 'customer_name', 'customer_name2', '192.168.238.172',
# 'administrator', 'Hzmc321#', '22', 'root', 'Abcd321#', '3307', 'windows测试环境')
